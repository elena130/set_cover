...
unsigned SetCover::fix_essential_columns(Assignment & assignment) {
...
unsigned SetCover::fix_out_dominated_rows(Assignment & assignment) {
    ...
    for(std::set<unsigned>::iterator i = uncovered_rows.begin(); i!=uncovered_rows.end(); ++i){
        ...
        for (unsigned k = 0; k < col_density[shortest]; ++k) {
            if (*i != ptr->row && assignment.rows[*i] == FREE && assignment.rows[ptr->row] == FREE) {
...
unsigned SetCover::fix_out_dominated_cols(Assignment & assignment) {
...
unsigned SetCover::fix_out_dominated_cols_set(Assignment & assignment) {
    ...
    for (std::set<unsigned>::iterator j = available_col.begin(); j != available_col.end(); ++j) {
        ...
        if (assignment.cols[*j] != FREE) {
            continue;
        }

        if (column_is_set_dominated(*j, assignment.cols)) {
            assignment.cols[*j] = FIX_OUT;
...
void SetCover::delete_fix_out_rows(Assignment& assignment) {
    for (unsigned i = 0; i < n_rows; ++i) {
        if(assignment.rows[i] == FIX_OUT)
            remove_row(i);
    }
}

void SetCover::delete_fix_out_cols(Assignment& assignment) {
    for(unsigned j=0; j< n_cols; ++j){
        if(assignment.cols[j] == FIX_OUT)
            remove_col(j);
    }
}
